...
const ProductCategoryStep = ({ categories, categoryId, onChange }: ProductCategoryStepProps) => {
  ...
  return <section className="product-wizard-step-content">
    ...
    <div className="product-wizard-category-grid">
      {roots.map((root) => <section className="product-wizard-category-group" key={root.id}><h4>{root.name}</h4><div>{categories.filter((category) => category.parentId === root.id && category.status === 'active').map((category) => <button type="button" className={categoryId === category.id ? 'is-selected' : ''} key={category.id} onClick={() => onChange(category.id, operationByKind(category.kind))}><strong>{category.name}</strong><span>{category.kind === 'esim' ? 'eSIM' : category.kind === 'physical_sim' ? 'SIM vật lý' : category.kind === 'topup' ? 'Nạp thêm' : category.kind === 'device' ? 'Thiết bị' : 'Phụ kiện'}</span></button>)}</div></section>)}
    </div>
