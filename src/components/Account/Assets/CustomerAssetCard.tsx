...
const paths: Record<string, string> = { ESIM: 'esim', PHYSICAL_SIM: 'sim-thiet-bi/sim', DEVICE: 'sim-thiet-bi/thiet-bi' };

export const CustomerAssetCard = ({ asset }: { asset: CustomerAsset }) => {
  if (asset.assetType === 'TOPUP' || !paths[asset.assetType]) return null;
  return <Link className="account-asset-card" to={`/tai-khoan/${paths[asset.assetType]}/${asset.id}`}>
