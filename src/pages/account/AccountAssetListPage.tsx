...
export const AccountAssetListPage = ({ title, lead, data, error, loading, reload, emptyLink = '/san-pham' }: { title: string; lead: string; data: CustomerAssetList | null; error: Error | null; loading: boolean; reload: () => void; emptyLink?: string }) => <>
  <div className="account-page-heading"><div><p className="account-kicker">Tai san</p><h2>{title}</h2><p className="account-lead">{lead}</p></div><Link className="account-button account-button-primary" to={emptyLink}>Mua them</Link></div>
