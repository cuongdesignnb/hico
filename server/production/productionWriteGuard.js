export const createProductionWriteGuard = ({ readinessService, env = process.env, allowWhenNotReady = () => false, isMaintenanceMutation = () => false } = {}) => async (req, res, next) => {
  if (['GET', 'HEAD', 'OPTIONS'].includes(req.method)) return next();
  if (env.NODE_ENV !== 'production') return next();
  if (allowWhenNotReady(req) || isMaintenanceMutation(req)) return next();
