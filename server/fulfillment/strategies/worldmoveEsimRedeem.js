  ...
  async callback({ item, event, providerClient, record }) {
    ...
    if (data.redemptionCode && !data.qrcode && providerClient?.redeem) {
      await providerClient.redeem({
        ...
        idempotencyKey: `${event.providerOrderId ?? event.orderId ?? record?.providerReference}:${item.variantId}:REDEEM`,
      });
      return result('PENDING_CALLBACK', { providerReference: event.providerOrderId ?? event.orderId ?? record?.providerReference, itemData: data });
    }
    return result('PROVISIONED', { providerReference: event.providerOrderId ?? event.orderId ?? record?.providerReference, itemData: data });
  },
});
