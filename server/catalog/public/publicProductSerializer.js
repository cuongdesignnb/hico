...
export const toPublicVariant = (variant, { providerOffers = [], productOperation } = {}) => {
  ...
  return {
    ...
    ...(variant.durationValue !== undefined ? { durationValue: variant.durationValue } : {}),
    ...(variant.durationUnit ? { durationUnit: variant.durationUnit } : {}),
    ...(Array.isArray(variant.tripDayOptions) ? { tripDayOptions: [...variant.tripDayOptions] } : {}),
    ...(typeof variant.cancellable === 'boolean' ? { cancellable: variant.cancellable } : {}),
...
export const toPublicProduct = (product, variants = [], { includeVariants = true, mediaAssets = [], providerOffers = [] } = {}) => {
  ...
  return {
    ...
    ...(Array.isArray(product.coverageDestinations) ? { coverageDestinations: product.coverageDestinations.map(({ id, name }) => ({ id, name })) } : {}),
